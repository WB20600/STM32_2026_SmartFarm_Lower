...
void Step_Motor_Init(void)
{
	...
	//步进电机A初始化（滑台）
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_8|GPIO_Pin_3;
	...
	//步进电机B初始化（丝杆）
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15|GPIO_Pin_14|GPIO_Pin_11;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100M
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
...
//	Stepper_Motor_Run_2(55);//上负下正
//	Stepper_Motor_Run_1(-8);//左正右负

	//Stepper_Motor_Run_2(-55);//上负下正
//	Stepper_Motor_Run_1(8);//左正右负
	Stepper_OFF();				//步进电机失能减少功耗
}
...
float sensor_sr_ultrasonic_read(void)
{
	...
	TIM_Cmd(TIM13, ENABLE);   // 使能TIMx外设
    while (ECHO_GET() == 0) /* 等待接收口高电平输出 */
        ;
    ...
    csb_t = TIM13->CNT;
	TIM_Cmd(TIM13, DISABLE); // 使能TIMx外设
    // 340m/s = 0.034cm/us
    if (csb_t < 25000)
    {
